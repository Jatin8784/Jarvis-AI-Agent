import { getMiniMaxClient, buildSystemPrompt } from "./minimax.client";
...
export async function runAgentMiniMax(
  userMessage: string,
  ...
  const model = (store.get("model") as string) || "MiniMax-M3";
  ...
  try {
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      console.log(`🔄 MiniMax round ${round + 1}`);
