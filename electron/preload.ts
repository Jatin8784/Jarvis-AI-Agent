...
contextBridge.exposeInMainWorld("jarvis", {
  ...
  // History
  getHistory: (limit?: number, userId?: string) =>
    ipcRenderer.invoke("db:history", limit, userId),
  clearHistory: () => ipcRenderer.invoke("db:clear"),
  createSession: (id: string, title: string, userId?: string) =>
    ipcRenderer.invoke("db:session-create", { id, title, userId }),
  updateSession: (id: string, title: string) =>
    ipcRenderer.invoke("db:session-update", { id, title }),
  ...
  saveAttachments: (data: any) =>
    ipcRenderer.invoke("db:save-attachments", data),
  ...
  transcribe: (audio: string, mimeType: string) =>
    ipcRenderer.invoke("agent:transcribe", { audio, mimeType }),
  checkWakeWord: (audio: string, mimeType: string) =>
    ipcRenderer.invoke("agent:wakeword", { audio, mimeType }),
  ...
  revertFileChange: (changeId: string) =>
    ipcRenderer.invoke("file:revert", changeId),
  ...
  verifyEmailOTP: (email: string, code: string) =>
    ipcRenderer.invoke("otp:verify-email", { email, code }),
